...
const MyPage = () => {
  ...
  }, [inputs.name, inputs.email, inputs.petName, inputs.breed, inputs.age, inputs.introduce])
    ...
    <div>
      ...
      <>
        ...
        <div className="UserPetInfo">
          ...
          <button className="editInfoBtn" onClick={onToggleOn}>정보 수정</button>
          </>
           :
          <>
           <div className="UserPetInfo">
            ...
            <input 
              id="petName"
              type="text"
              placeholder={me?.pet?.petName ? `펫 이름: ${me?.pet?.petName}` : '펫 이름을 입력해주세요'}
            ...
            <input 
              id="breed"
              type="text"
              placeholder={me?.pet?.breed ? `펫의 종: ${me?.pet?.breed}` : '애완동물의 종을 입력해주세요'}
              value={inputs.breed}
              onChange={onEditInfo}
            />
            <input 
              id="age"
              type="number"
              placeholder={me?.pet?.age?.toString() ? `펫 나이: ${me?.pet?.age.toString()}` : '펫 나이을 입력해주세요'}
